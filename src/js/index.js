...
const setStyle = () => {
  const { red, green, blue } = inputValues[currentStyleType];
  textContainer.style[currentStyleType] = `rgb(${red}, ${green}, ${blue})`;
};
...
const handleRangeControllers = (ranges) => {
  ...
  rangesList.forEach((range, i) => {
    $(`#slider-${i + 1}`).slider({
      ...
      classes: {
        "ui-slider": "highlight",
        "ui-slider-handle": `ui-slider-handle--${colorsList[i]}`,
        "ui-slider-range": `ui-slider-range--${colorsList[i]}`
      },
